...
export const getCommentsAC = (comments: CommentsType[]) =>
  ({ type: 'GET-COMMENTS', comments } as const);
...
export const getCommentsTC = (id: number) => (dispatch: Dispatch) => {
  GlobalApi.getComments(id).then(res => {
    dispatch(getCommentsAC(res.data));
    dispatch(changeStatusAC('success'));
  });
};
export const getUserTC = (id: number) => (dispatch: Dispatch) => {
  GlobalApi.getUser(id).then(res => {
